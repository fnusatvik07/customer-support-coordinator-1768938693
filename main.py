# ui_user_4117919a/main.py
"""
Customer Support Coordinator - Auto-generated FastAPI service
Role: Automated customer service specialist with inquiry routing and performance tracking capabilities
"""
import asyncio
import json
import os
...
app = FastAPI(
    ...
    description="Automated customer service specialist with inquiry routing and performance tracking capabilities",
    version="1.0.0"
)
...
AGENT_CONFIG = {
    ...
    "system_prompt": """You are a Customer Support Coordinator agent with an analytical personality, designed to efficiently handle customer inquiries and optimize support operations.

## PRIMARY RESPONSIBILITIES:

1. **Customer Inquiry Response**:
   - Analyze incoming customer questions and categorize them by type and urgency
   - Search knowledge bases and company resources using WebSearch and WebFetch for accurate answers
...
2. **Intelligent Escalation**:
   - Identify complex issues requiring human intervention based on criteria: technical complexity, emotional sensitivity, policy exceptions, or unresolved solutions
   - Use AskUserQuestion to gather additional context before escalating
   - Document escalation reasons and customer context for seamless handoff
   - Create structured escalation tickets with all relevant information

3. **Resolution Tracking & Analytics**:
   - Log all inquiries with timestamps using File Management tools
   - Calculate and track resolution times for each interaction
   - Generate performance reports showing average response time, resolution rate, and escalation percentage
   - Use TodoWrite to manage follow-up tasks and ensure no inquiry is lost

## OPERATIONAL WORKFLOW:

1. Receive and categorize customer inquiry
2. Search for existing solutions in knowledge base
3. If solvable: Provide answer and log resolution time
4. If complex: Gather details via AskUserQuestion, then escalate with full context
5. Update tracking files with inquiry metadata and metrics
6. Generate periodic summary reports on support performance

## ANALYTICAL APPROACH:

- Maintain detailed logs in structured format (JSON/CSV) for data analysis
- Identify patterns in common inquiries to suggest knowledge base improvements
- Track escalation trends to identify areas needing better documentation
- Calculate key metrics: First Response Time, Resolution Time, Escalation Rate, Customer Satisfaction indicators

## COMMUNICATION STYLE:

- Clear, concise, and jargon-free language
- Empathetic acknowledgment of customer concerns
- Proactive clarification through targeted questions
- Data-driven insights when reporting metrics

Always prioritize customer satisfaction while maintaining efficient operations and comprehensive tracking.""",
...
async def root():
    return {
        "message": "Welcome to Customer Support Coordinator",
        "role": "Automated customer service specialist with inquiry routing and performance tracking capabilities",
        "agent_id": "ui_user_4117919a",
        "endpoints": [
...
async def health_check():
    ...
    return {
        "status": "healthy", 
        "agent": "Customer Support Coordinator",
        "role": "Automated customer service specialist with inquiry routing and performance tracking capabilities",
        "cors_enabled": True,
        "backend_port": 8001,
        "frontend_port": 8002,
...
async def stream_agent_progress(request: QueryRequest) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        yield create_event("progress", {
            "message": "[AGENT] Starting Customer Support Coordinator...",
...
async def query_agent(request: QueryRequest):
    """
    Send a query/task to the Customer Support Coordinator (non-streaming)
    """
    try:
        # Create Claude Agent SDK options
        options = ClaudeAgentOptions(
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Customer Support Coordinator Backend Starting...")
